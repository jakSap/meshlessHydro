...
def createPlot(h5File, outDir, plotGrad, plotVel, iNNL):
    ...
    if "Ghosts" not in str(h5File):
        ...
        ax.set_xlim((0., 1.))
        ax.set_ylim((0., 1.))
    #uncomment below when ghost particles are not plotted
    ax.set_xlim((-.2, 1.2))
    ax.set_ylim((-.2, 1.2))
...
def plotNNL(h5File, iNNL, pos, ax):
    ...
    ax.scatter(pos[iNNL,0], pos[iNNL,1], s=50., marker='x', color='r')
    ax.scatter(posNNL[:,0], posNNL[:,1], s=50, marker='x', color='m')
...
def createPressurePlot(h5File, outDir):
    ...
    PPlt = ax.scatter(pos[:,0], pos[:,1], c=P, s=5.) # good for 10**4 particles
    ...
    plt.title(r"Color coded pressure $P$ at $t = " + str(time) + r"$")
    plt.xlabel("$x$")
    plt.ylabel("$y$")    
