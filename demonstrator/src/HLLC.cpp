...
//
// Portions of this file were adapted from https://gitlab.cosma.dur.ac.uk/swift/swiftsim
// They were published under the GNU General Public License, as is this project.
// 
// Original header:
...
#if MURNAGHAN_EOS
// Riemann HLLC adapted from swift for a Murnaghan EOS.
// Ignores energy flux (because iothermal EOS) and uses Einfeldt wave speed estimates.
...
void HLLC::solveHLLC1(double *WR, double *WL, double *n,
    double *totflux, const double *vij,
    const double &MURN_K0, const double &MURN_n, const double &MURN_rho0){
    ...
    const double aL = MURN_K0 / MURN_n * pow(WL[1] / MURN_rho0, MURN_n - 1);
    const double aR = MURN_K0 / MURN_n * pow(WR[1] / MURN_rho0, MURN_n - 1);
